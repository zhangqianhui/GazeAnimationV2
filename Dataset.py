...
class Dataset(object):
    ...
    def __init__(self, config):
        ...
        self.train2_list, self.train_mask2_list = self.filenames2_list, self.mask2
        self.test_list, self.test_mask_list = self.filenames2_list, self.mask2
    ...
    def readfilenames(self):
        ...
        filenames2 = glob.glob(os.path.join(self.data_dir, "test_eye_rotation_dataset" + "/*.jpg"))
        mask2 = []
        middle_2 = []
        for j, pathname in enumerate(filenames2):
            newpathname = pathname.replace("test_eye_rotation_dataset", "test_eye_rotation_dataset_mask")
            if os.path.exists(newpathname):
    ...
    def Get_queues_image_mask(self, list1, list2, is_shuffle):
        ...
        masks_queue2 = self.read_images(input_queue=input_queue1[1], is_mask=True)

        return images_queue1, masks_queue2

    def input(self):

        images_queue1, masks_queue1 = self.Get_queues_image_mask(self.train1_list, self.train_mask1_list, is_shuffle=True)
        images_queue2, masks_queue2 = self.Get_queues_image_mask(self.train2_list, self.train_mask2_list, is_shuffle=True)
        images_queue3, masks_queue3 = self.Get_queues_image_mask(self.test_list, self.test_mask_list, is_shuffle=False)
